...
{
file_name = dictionray + "test_scene";
read_file(file_name);
if_bounding = 1;
if (if_bounding) {
    normalize();
...
void key(unsigned char ch, int x, int y) {
    ...
    switch (ch) {
        ...
        case 'T':
            ...
            cout << "Do you need bounding box? (Input '0' for 'no' or '1' for 'yes')" << endl;
            cin >> if_bounding;
            if (if_bounding) {
                normalize();
            }
        ...
        case 'F':
            ...
            cout << "Please input the file name('test_scene' or 'bunny_scene') that stores your polygons. \n"
                 << "The file should be put under the main dictionary.('Project 2') \n "
                 << endl;
            cin >> file_name;
